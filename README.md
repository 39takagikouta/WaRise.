...
- 会員登録
- ログイン
- お笑いの嗜好性（好みや動画の長さ等）入力機能（この機能をどのような方法で実現するか悩んでいます）
- 翌日に起床する時間を設定する機能（報酬としてレコメンドされた動画を見ることもできるし、自身でカスタムすることもできる）
- 翌朝、動画のロックを解除する機能（設定した時間になってから 10 分は解除可能）
- 動画が気に入らなかったら、他の動画をレコメンドしてくれる機能
- 起床成功後に試聴した動画と試聴成功をポスト（ツイート）できる機能
- 失敗した際の機能（何を表示するか迷っています。youtbe でユーザーを慰める動画を探してそれを表示するのが良いかなと考えています）
- 起床時間と起床成功 or 失敗を記録し表で振り返ることができる機能
- 起床成功回数、成功率、連続成功数のランキング（起床時間は自分で設定したもので良い）機能(ランキング参加は任意)
